# -*- coding: utf-8 -*-
"""BCI_TL_Cross_Subject_EEG_ITNet.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1tJhcWBEibXWB1bWCl6EplkiQDflJQKYi
"""
...
def modeling_EEG_ITNet(test_data_half1, test_labels_half1, test_data_half2, test_labels_half2, train_data, train_labels, participant_num, Chans, Samples):

    # Print data shapes
    print("   **********   ")

    # Build and compile the EEG_ITNet model
    ...
    # Train the EEG_ITNet model on train_data
    ...
    # Load the create_frozen_EEG_ITNet model and transfer weights
    frozen_model = create_frozen_EEG_ITNet(Chans, Samples)
    frozen_model.load_weights(model_filename)

    # Train create_frozen_EEG_ITNet on test_data_half1 and test_labels_half1
    frozen_model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    fitted_frozen = frozen_model.fit(x=test_data_half1, y=test_labels_half1, epochs=400, validation_split=0.2, verbose=1, batch_size=128)
    plot_training_history(fitted_frozen)
    tf.keras.backend.clear_session()

    # Evaluate create_frozen_EEG_ITNet on test_data_half2
    predicted_frozen = frozen_model.predict(x=test_data_half2)
    predicted_labels_multiclass_frozen = np.argmax(predicted_frozen, axis=1)

    # Calculate metrics for create_frozen_EEG_ITNet
    accuracy_frozen = metrics.accuracy_score(np.argmax(test_labels_half2, axis=1), predicted_labels_multiclass_frozen)
    print("Accuracy for create_frozen_EEG_ITNet:", accuracy_frozen)

    f1_score_frozen = metrics.f1_score(np.argmax(test_labels_half2, axis=1), predicted_labels_multiclass_frozen, average='weighted')
    print("F1-score for create_frozen_EEG_ITNet:", f1_score_frozen)

    classification_report_frozen = metrics.classification_report(np.argmax(test_labels_half2, axis=1), predicted_labels_multiclass_frozen)
    print("Classification Report for create_frozen_EEG_ITNet:\n", classification_report_frozen)
...
test_data_half1 = test_data[:len(test_data)//2]
test_data_half2 = test_data[len(test_data)//2:]
test_labels_half1 = test_labels[:len(test_labels)//2]
test_labels_half2 = test_labels[len(test_labels)//2:]


modeling_EEG_ITNet(test_data_half1, test_labels_half1, test_data_half2, test_labels_half2, train_data, train_labels, participant_num, Chans, n_chunks)
...
def save_chunks_in_file(participant_total, n_chunks, participant_num):
    ...
    results_per_participant_file_path = os.path.join(GOOGLE_DRIVE_PATH, 'results_per_participant.pkl')
    # Save the results_per_participant object to a file using pickle
    with open(results_per_participant_file_path, 'wb') as file:
        pickle.dump(results_per_participant, file)

    print(f'Saved results_per_participant.pkl in the path: {results_per_participant_file_path}')
